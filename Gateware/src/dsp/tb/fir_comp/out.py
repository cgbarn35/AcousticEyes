...
with open(path+'pdm.csv', 'r') as f:
    ...
    fir_2= np.asarray([float(row[2]) for row in data], dtype=float)
...
plt.step(t_v*1e-4, fir_2/2**16, label='FIR Multi MAC',  linewidth=1.5)
plt.step(t_v*1e-4, fir_1/2**16, label='FIR Single MAC',  linewidth=1.5)
plt.xlabel('Time (ms)')
plt.ylim(-0.05,1.05)
plt.legend()
plt.show()
